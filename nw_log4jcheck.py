    ...
    for url in urls:
        ...
        with open('/var/log/named/query.log') as f:
            if f"{url_identifiers[url]}" in f.read():
                ...
                logging.info(f"NO VULNERABILITY DETECTED! Proceed with on-server checking. No incoming DNS request to {url_identifiers[url]}.{HOSTNAME} was seen while checking system at {url}")
